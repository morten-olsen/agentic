...
const rowToInterrupt = (row: InterruptRow): Interrupt => {
  return {
    ...
    skillActivation: row.skill_activation ? JSON.parse(row.skill_activation) : undefined,
...
const createInterrupt = async (db: Knex, input: CreateInterruptInput): Promise<Interrupt> => {
  ...
  const row: InterruptRow = {
    ...
    skill_activation: validated.skillActivation ? JSON.stringify(validated.skillActivation) : null,
    status: 'pending',

...
class KnexStore extends BaseStore {
  ...
  #verifyVectorSearchTable = async (): Promise<void> => {
    ...
    const tableExists = await this.#withRawConnection((db) => {
      return db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='vec_items'").get() as
        | { name: string }
        | undefined;
    });

    if (!tableExists) {
      throw new VectorSearchNotInitializedError('vec_items table not found. Run database migrations to create it.');
    }
  ...
  async #searchWithVec(
    ...
    const indexRows = await this.#db()('store_embedding_index')
      .whereIn('rowid', rowids)
      .select('rowid', 'namespace', 'key');

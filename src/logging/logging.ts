...
import type { LogLevel, LogEntry, LogContext, LogQueryFilters, LogStats, LogConfig, LogConfigInput } from './logging.schemas.ts';
...
const extractErrorDetails = (
  ...
  for (const key of Object.keys(error)) {
    if (!['name', 'message', 'stack', 'cause', 'response', 'request', 'code', 'status', 'statusCode', 'error'].includes(key)) {
      try {
